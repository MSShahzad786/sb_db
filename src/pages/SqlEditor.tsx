...
export default function SqlEditor() {
  ...
  const saveToHistory = (sql: string) => {
    const newHistory = [sql, ...queryHistory.slice(0, 19)]
    ...
    <div className="h-full flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={executeQuery} disabled={loading} className="shadow-lg shadow-primary/20">
      ...
      <div className="grid grid-cols-12 gap-6 flex-1 min-h-0">
        ...
        <div className="col-span-3 flex flex-col gap-6 overflow-hidden">
          <Card className="border-primary/10 glassmorphism flex flex-col max-h-[50%]">
          ...
          <Card className="border-primary/10 glassmorphism flex flex-col flex-1 overflow-hidden">
            <CardHeader className="py-3 border-b">
              <CardTitle className="text-sm font-bold flex items-center gap-2">
            ...
            <CardContent className="p-2 overflow-y-auto">
              <div className="space-y-1">
                {queryHistory.map((sql, i) => (
                  <div key={i} className="p-2 hover:bg-muted rounded-md transition-colors cursor-pointer" onClick={() => setQuery(sql)}>
                    <p className="text-xs font-mono text-muted-foreground line-clamp-2">{sql}</p>
                  </div>
                ))}
